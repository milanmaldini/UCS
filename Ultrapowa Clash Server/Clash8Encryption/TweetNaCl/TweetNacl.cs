...
namespace UCS.EncryptionTesting
{
	 ...
	 * */
	public sealed class TweetNacl
	{

		private const string TAG = "TweetNacl";
		...
		public sealed class Box
		{
			...
			internal const string TAG = "Box";

			internal long nonce;

			internal byte[] theirPublicKey;
			internal byte[] mySecretKey;
			internal byte[] sharedKey;

			public Box(byte[] theirPublicKey, byte[] mySecretKey) : this(theirPublicKey, mySecretKey, 68)
			{
			}
			...
			internal byte[] generateNonce()
			{
				...
				{
					n[i + 0] = (byte)((long)((ulong)nonce >> 0));
					n[i + 1] = (byte)((long)((ulong)nonce >> 8));
					n[i + 2] = (byte)((long)((ulong)nonce >> 16));
					n[i + 3] = (byte)((long)((ulong)nonce >> 24));
					n[i + 4] = (byte)((long)((ulong)nonce >> 32));
					n[i + 5] = (byte)((long)((ulong)nonce >> 40));
					n[i + 6] = (byte)((long)((ulong)nonce >> 48));
					n[i + 7] = (byte)((long)((ulong)nonce >> 56));
				}
			...
			public byte [] box(byte[] message, byte[] theNonce)
			{
				...
				// check message
				if (!(message != null && message.Length > 0 && theNonce != null && theNonce.Length == nonceLength))
				{
					return null;
				}
			...
			{
				// check message
				if (!(box != null && box.Length > boxzerobytesLength && theNonce != null && theNonce.Length == nonceLength))
				{
					return null;
				}
			 ...
			 * */
			public const int sharedKeyLength = 32;
			 ...
			 * */
			public const int zerobytesLength = 32;
			/*
			 * @description
			 *   zero bytes in case open box
			 * */
			public const int boxzerobytesLength = 16;

			/*
			 * @description 
			 *   Length of overhead added to box compared to original message.
			 * */
			public const int overheadLength = 16;
			...
			public static KeyPair keyPair()
			{
				...
				crypto_box_keypair(kp.PublicKey, kp.SecretKey);
				return kp;
			}
		...
		public sealed class SecretBox
		{
			...
			internal const string TAG = "SecretBox";

			internal long nonce;

			internal byte[] key;

			public SecretBox(byte[] key) : this(key, 68)
			{
			}
		 ...
		 * */
		public sealed class ScalarMult
		{

			internal const string TAG = "ScalarMult";
			 ...
			 * */
			public const int groupElementLength = 32;
		...
		public sealed class Hash
		{
			...
			internal const string TAG = "Hash";
		...
		public sealed class Signature
		{
			...
			internal const string TAG = "Signature";
			 ...
			 * */
			public bool detached_verify(byte[] message, byte[] signature)
			{
			...
			public static KeyPair keyPair()
			{
				...
				crypto_sign_keypair(kp.PublicKey, kp.SecretKey, false);
				return kp;
			}
			...
			public static KeyPair keyPair_fromSecretKey(byte[] secretKey)
			{
				...
				// copy sk
				for (int i = 0; i < kp.SecretKey.Length; i++)
				{
					sk[i] = secretKey[i];
				}
		...
		private static readonly byte[] _9 = new byte[32];
		static TweetNacl()
		{
		...
		private static readonly long[] gf1 = new long[16];
		private static readonly long[] _121665 = new long[16];
		...
		private static readonly long[] X = new long [] {0xd51a, 0x8f25, 0x2d60, 0xc956, 0xa7b2, 0x9525, 0xc760, 0x692c, 0xdc5c, 0xfdd6, 0xe231, 0xc0a4, 0x53fe, 0xcd6e, 0x36d3, 0x2169};
		private static readonly long[] Y = new long [] {0x6658, 0x6666, 0x6666, 0x6666, 0x6666, 0x6666, 0x6666, 0x6666, 0x6666, 0x6666, 0x6666, 0x6666, 0x6666, 0x6666, 0x6666, 0x6666};
		...
		{
			return (x << c) | ((int)((uint)(x & 0xffffffff) >> (32 - c)));
		}
		...
		private static void st32(byte[] x, int xoff, int xlen, int u)
		{
			...
			for (i = 0; i < 4; i++)
			{
				x[i + xoff] = unchecked((byte)(u & 0xff));
				u = (int)((uint)u >> 8);
			}
		}
		...
		private static readonly byte[] sigma = new byte[] {101, 120, 112, 97, 110, 100, 32, 51, 50, 45, 98, 121, 116, 101, 32, 107};
		/*static {
			try {
		...
		private static int crypto_stream_salsa20_xor(byte[] c, byte[] m, long b, byte[] n, int noff, int nlen, byte[] k)
		{
			...
			while (b >= 64)
			{
				...
				for (i = 0; i < 64; i++)
				{
					c[i + coffset] = unchecked((byte)(((m != null?m[i + moffset]:0) ^ x[i]) & 0xff));
				}
				u = 1;
				for (i = 8;i < 16;++i)
				{
					u += (int)(z[i] & 0xff);
					z[i] = unchecked((byte)(u & 0xff));
					u = (int)((uint)u >> 8);
				}
				b -= 64;
				coffset += 64;
		...
		*/
		public sealed class poly1305
		{
			...
			public poly1305(byte[] key)
			{
				...
				this.r[1] = (((int)((uint)t0 >> 13)) | (t1 << 3)) & 0x1fff;
				t2 = key[4] & 0xff | (key[5] & 0xff) << 8;
				this.r[2] = (((int)((uint)t1 >> 10)) | (t2 << 6)) & 0x1f03;
				t3 = key[6] & 0xff | (key[7] & 0xff) << 8;
				this.r[3] = (((int)((uint)t2 >> 7)) | (t3 << 9)) & 0x1fff;
				t4 = key[8] & 0xff | (key[9] & 0xff) << 8;
				this.r[4] = (((int)((uint)t3 >> 4)) | (t4 << 12)) & 0x00ff;
				this.r[5] = (((int)((uint)t4 >> 1))) & 0x1ffe;
				t5 = key[10] & 0xff | (key[11] & 0xff) << 8;
				this.r[6] = (((int)((uint)t4 >> 14)) | (t5 << 2)) & 0x1fff;
				t6 = key[12] & 0xff | (key[13] & 0xff) << 8;
				this.r[7] = (((int)((uint)t5 >> 11)) | (t6 << 5)) & 0x1f81;
				t7 = key[14] & 0xff | (key[15] & 0xff) << 8;
				this.r[8] = (((int)((uint)t6 >> 8)) | (t7 << 8)) & 0x1fff;
				this.r[9] = (((int)((uint)t7 >> 5))) & 0x007f;
			...
			public poly1305 blocks(byte[] m, int mpos, int bytes)
			{
				...
				while (bytes >= 16)
				{
					...
					h1 += (((int)((uint)t0 >> 13)) | (t1 << 3)) & 0x1fff;
					t2 = m[mpos + 4] & 0xff | (m[mpos + 5] & 0xff) << 8;
					h2 += (((int)((uint)t1 >> 10)) | (t2 << 6)) & 0x1fff;
					t3 = m[mpos + 6] & 0xff | (m[mpos + 7] & 0xff) << 8;
					h3 += (((int)((uint)t2 >> 7)) | (t3 << 9)) & 0x1fff;
					t4 = m[mpos + 8] & 0xff | (m[mpos + 9] & 0xff) << 8;
					h4 += (((int)((uint)t3 >> 4)) | (t4 << 12)) & 0x1fff;
					h5 += (((int)((uint)t4 >> 1))) & 0x1fff;
					t5 = m[mpos + 10] & 0xff | (m[mpos + 11] & 0xff) << 8;
					h6 += (((int)((uint)t4 >> 14)) | (t5 << 2)) & 0x1fff;
					t6 = m[mpos + 12] & 0xff | (m[mpos + 13] & 0xff) << 8;
					h7 += (((int)((uint)t5 >> 11)) | (t6 << 5)) & 0x1fff;
					t7 = m[mpos + 14] & 0xff | (m[mpos + 15] & 0xff) << 8;
					h8 += (((int)((uint)t6 >> 8)) | (t7 << 8)) & 0x1fff;
					h9 += (((int)((uint)t7 >> 5))) | hibit;
			...
			public poly1305 finish(byte[] mac, int macpos)
			{
				...
				c = (int)((uint)this.h[1] >> 13);
				this.h[1] &= 0x1fff;
				for (i = 2; i < 10; i++)
				{
					this.h[i] += c;
					c = (int)((uint)this.h[i] >> 13);
					this.h[i] &= 0x1fff;
				}
				this.h[0] += (c * 5);
				c = (int)((uint)this.h[0] >> 13);
				this.h[0] &= 0x1fff;
				this.h[1] += c;
				c = (int)((uint)this.h[1] >> 13);
				this.h[1] &= 0x1fff;
				...
				this.h[1] = (((int)((uint)this.h[1] >> 3)) | (this.h[2] << 10)) & 0xffff;
				this.h[2] = (((int)((uint)this.h[2] >> 6)) | (this.h[3] << 7)) & 0xffff;
				this.h[3] = (((int)((uint)this.h[3] >> 9)) | (this.h[4] << 4)) & 0xffff;
				this.h[4] = (((int)((uint)this.h[4] >> 12)) | (this.h[5] << 1) | (this.h[6] << 14)) & 0xffff;
				this.h[5] = (((int)((uint)this.h[6] >> 2)) | (this.h[7] << 11)) & 0xffff;
				this.h[6] = (((int)((uint)this.h[7] >> 5)) | (this.h[8] << 8)) & 0xffff;
				this.h[7] = (((int)((uint)this.h[8] >> 8)) | (this.h[9] << 5)) & 0xffff;

				f = this.h[0] + this.pad[0];
				this.h[0] = f & 0xffff;
				for (i = 1; i < 8; i++)
				{
					f = (((this.h[i] + this.pad[i]) | 0) + ((int)((uint)f >> 16))) | 0;
					this.h[i] = f & 0xffff;
				}

				mac[macpos + 0] = unchecked((byte)(((int)((uint)this.h[0] >> 0)) & 0xff));
				mac[macpos + 1] = unchecked((byte)(((int)((uint)this.h[0] >> 8)) & 0xff));
				mac[macpos + 2] = unchecked((byte)(((int)((uint)this.h[1] >> 0)) & 0xff));
				mac[macpos + 3] = unchecked((byte)(((int)((uint)this.h[1] >> 8)) & 0xff));
				mac[macpos + 4] = unchecked((byte)(((int)((uint)this.h[2] >> 0)) & 0xff));
				mac[macpos + 5] = unchecked((byte)(((int)((uint)this.h[2] >> 8)) & 0xff));
				mac[macpos + 6] = unchecked((byte)(((int)((uint)this.h[3] >> 0)) & 0xff));
				mac[macpos + 7] = unchecked((byte)(((int)((uint)this.h[3] >> 8)) & 0xff));
				mac[macpos + 8] = unchecked((byte)(((int)((uint)this.h[4] >> 0)) & 0xff));
				mac[macpos + 9] = unchecked((byte)(((int)((uint)this.h[4] >> 8)) & 0xff));
				mac[macpos + 10] = unchecked((byte)(((int)((uint)this.h[5] >> 0)) & 0xff));
				mac[macpos + 11] = unchecked((byte)(((int)((uint)this.h[5] >> 8)) & 0xff));
				mac[macpos + 12] = unchecked((byte)(((int)((uint)this.h[6] >> 0)) & 0xff));
				mac[macpos + 13] = unchecked((byte)(((int)((uint)this.h[6] >> 8)) & 0xff));
				mac[macpos + 14] = unchecked((byte)(((int)((uint)this.h[7] >> 0)) & 0xff));
				mac[macpos + 15] = unchecked((byte)(((int)((uint)this.h[7] >> 8)) & 0xff));
		...
		public static int crypto_scalarmult(byte[] q, byte[] n, byte[] p)
		{
			...
			z[31] = unchecked((byte)(((n[31] & 127) | 64) & 0xff));
			z[0] &= unchecked((byte)248);
			...
			for (i = 254;i >= 0;--i)
			{
				r = ((int)((uint)z[(int)((uint)i >> 3)] >> (i & 7))) & 1;
		...
		private static readonly byte[] iv = new byte[] {0x6a,0x09,unchecked((byte) 0xe6),0x67,unchecked((byte) 0xf3),unchecked((byte) 0xbc),unchecked((byte) 0xc9),0x08, unchecked((byte) 0xbb),0x67,unchecked((byte) 0xae),unchecked((byte) 0x85),unchecked((byte) 0x84),unchecked((byte) 0xca),unchecked((byte) 0xa7),0x3b, 0x3c,0x6e,unchecked((byte) 0xf3),0x72,unchecked((byte) 0xfe),unchecked((byte) 0x94),unchecked((byte) 0xf8),0x2b, unchecked((byte) 0xa5),0x4f,unchecked((byte) 0xf5),0x3a,0x5f,0x1d,0x36,unchecked((byte) 0xf1), 0x51,0x0e,0x52,0x7f,unchecked((byte) 0xad),unchecked((byte) 0xe6),unchecked((byte) 0x82),unchecked((byte) 0xd1), unchecked((byte) 0x9b),0x05,0x68,unchecked((byte) 0x8c),0x2b,0x3e,0x6c,0x1f, 0x1f,unchecked((byte) 0x83),unchecked((byte) 0xd9),unchecked((byte) 0xab),unchecked((byte) 0xfb),0x41,unchecked((byte) 0xbd),0x6b, 0x5b,unchecked((byte) 0xe0),unchecked((byte) 0xcd),0x19,0x13,0x7e,0x21,0x79};
		...
		private static int crypto_hash(byte[] @out, byte[] m, int moff, int mlen, int n)
		{
			...
			x[n] = unchecked((byte) 128);

			n = 256 - 128 * (n < 112?1:0);
			x[n - 9] = (byte)((long)((ulong)b >> 61));
		...
		public static int crypto_sign_keypair(byte[] pk, byte[] sk, bool seeded)
		{
			byte[] d = new byte[64];
			long[][] p = new long [4][];
		...
		private static void modL(byte[] r, int roff, int rlen, long[] x)
		{
			...
			{
				x[i + 1] += x[i] >> 8;
				r[i + roff] = unchecked((byte)(x[i] & 255));
			}
		}
		...
		public static int crypto_sign(byte[] sm, long dummy, byte[] m, int n, byte[] sk) //long -  *smlen not used
		{
		...
		public static int crypto_sign_open(byte[] m, long dummy, byte[] sm, int n, byte[] pk) //long -  *mlen not used
		{
			int i;
		 ...
		 * */
		private static readonly Random jrandom = new Random();
		...
		public static void randombytes(byte[] x, int len)
		{
			...
			for (int i = 0; i < len - ret; i += 8)
			{
				...
				x[i + 0] = (byte)((long)((ulong)rnd >> 0));
				x[i + 1] = (byte)((long)((ulong)rnd >> 8));
				x[i + 2] = (byte)((long)((ulong)rnd >> 16));
				x[i + 3] = (byte)((long)((ulong)rnd >> 24));
				x[i + 4] = (byte)((long)((ulong)rnd >> 32));
				x[i + 5] = (byte)((long)((ulong)rnd >> 40));
				x[i + 6] = (byte)((long)((ulong)rnd >> 48));
				x[i + 7] = (byte)((long)((ulong)rnd >> 56));
			}
