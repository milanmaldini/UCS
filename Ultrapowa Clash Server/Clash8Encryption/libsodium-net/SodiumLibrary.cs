...
namespace Sodium
{
  ...
  public static class SodiumLibrary
  {
    ...
    // Summary's by ExPl0itR

    /// <summary>
    /// Generates a 32 byte nonce
    /// </summary>
    /// <param name="outKey">Generated nonce will be stored here.</param>
    /// <param name="publicKey">The public key</param>
    /// <param name="secretKey">The secret key</param>
    /// <returns>0 = Successfully generated, -1 = Failed</returns>
    internal delegate int _BeforeNm(byte[] outKey, byte[] publicKey, byte[] secretKey);
    internal static LazyInvoke<_BeforeNm> _crypto_box_beforenm = new LazyInvoke<_BeforeNm>("crypto_box_beforenm", SodiumLibrary.Name);
    internal static _BeforeNm crypto_box_beforenm { get { return _crypto_box_beforenm.Method; } }

    /// <summary>
    /// Encrypts a packet (NEEDED!)
    /// </summary>
    /// <param name="cipherText">Output buffer</param>
    /// <param name="message">To be encrypted message</param>
    /// <param name="messageLen">Length of the encrypted message</param>
    /// <param name="nonce">The 24 byte nonce</param>
    /// <param name="sharedkey">The 32 byte nonce</param>
    /// <returns>0 = Successfully encrypted, -1 = Failed to encrypt</returns>
    internal delegate int _AfterNm(byte[] cipherText, byte[] message, Int32 messageLen, byte[] nonce, byte[] sharedkey );
    internal static LazyInvoke<_AfterNm> _crypto_box_easy_afternm = new LazyInvoke<_AfterNm>("crypto_box_easy_afternm", SodiumLibrary.Name);
    internal static _AfterNm crypto_box_easy_afternm { get { return _crypto_box_easy_afternm.Method; } }
    /// <summary>
    /// Decrypts a packet (NEEDED!), easy afternm takes 5 args, default afternm 6.. weird :(
    /// </summary>
    /// <param name="outBuffer">Decrypted text is stored here</param>
    /// <param name="cipherText">Encrypted text</param>
    /// <param name="cipherTextLen">Encrypted text length</param>
    /// <param name="nonce">24 byte nonce</param>
    /// <param name="sharedKey">32 byte nonce (sharedkey)</param>
    /// <returns>0 = Successfully decrypted, -1 = Failed to decrypt</returns>
    internal delegate int _AfterNmOpen(byte[] outBuffer, byte[] cipherText, Int64 cipherTextLen, byte[] nonce, byte[] sharedKey);
    internal static LazyInvoke<_AfterNmOpen> _crypto_box_open_easy_afternm = new LazyInvoke<_AfterNmOpen>("crypto_secretbox_xsalsa20poly1305_open", SodiumLibrary.Name);
    internal static _AfterNmOpen crypto_box_open_easy_afternm { get { return _crypto_box_open_easy_afternm.Method; } }

    [DllImport("libsodium.dll")]
    public static extern int crypto_secretbox_xsalsa20poly1305_open(byte[] outBuf, byte[] ciph, int ciphLen, byte[] nonce, byte[] sk);
    /// <summary>
    /// Returns specified random bytes (Used for the snonce)
    /// </summary>
