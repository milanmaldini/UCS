...
namespace UCS.EncryptionTesting
{
    internal class Curve25519
    {
        ...
        public static byte[] ClampPrivateKey(byte[] rawKey)
        {
            ...
            if (rawKey.Length != 32)
                throw new ArgumentException(
                    string.Format("rawKey must be 32 bytes long (but was {0} bytes long)", rawKey.Length), "rawKey");
        ...
        public static void ClampPrivateKeyInline(byte[] key)
        {
            ...
            if (key.Length != 32)
                throw new ArgumentException(string.Format("key must be 32 bytes long (but was {0} bytes long)",
                    key.Length));
        ...
        public static void KeyGenInline(byte[] publicKey, byte[] signingKey, byte[] privateKey)
        {
            ...
            if (publicKey.Length != 32)
                throw new ArgumentException(
                    string.Format("publicKey must be 32 bytes long (but was {0} bytes long)", publicKey.Length),
                    "publicKey");

            if (signingKey == null) throw new ArgumentNullException("signingKey");
            if (signingKey.Length != 32)
                throw new ArgumentException(
                    string.Format("signingKey must be 32 bytes long (but was {0} bytes long)", signingKey.Length),
                    "signingKey");

            if (privateKey == null) throw new ArgumentNullException("privateKey");
            if (privateKey.Length != 32)
                throw new ArgumentException(
                    string.Format("privateKey must be 32 bytes long (but was {0} bytes long)", privateKey.Length),
                    "privateKey");
        ...
        private static void Core(byte[] publicKey, byte[] signingKey, byte[] privateKey, byte[] peerPublicKey)
        {
            ...
            if (signingKey != null && signingKey.Length != 32)
                throw new ArgumentException(
                    string.Format("signingKey must be null or 32 bytes long (but was {0} bytes long)", signingKey.Length),
                    "signingKey");
            ...
            if (peerPublicKey != null && peerPublicKey.Length != 32)
                throw new ArgumentException(
                    string.Format("peerPublicKey must be null or 32 bytes long (but was {0} bytes long)",
                        peerPublicKey.Length), "peerPublicKey");
